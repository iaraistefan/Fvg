...
class PositionGuardian:
    ...
    def check_positions(self, active_positions: dict) -> int:
        ...
        for symbol, pos in list(active_positions.items()):
            ...
            pos_amt    = float(real_pos.get("positionAmt", 0))

            # ── EMERGENCY CLOSE ───────────────────────────────────
            # Folosim unRealizedProfit direct — cel mai precis
            try:
                unrealized   = float(real_pos.get("unRealizedProfit", 0))
                lev          = getattr(config, "LEVERAGE", 10)
                upt          = getattr(config, "USDT_PER_TRADE", 7)
                # ROI Binance = unrealizedProfit / marja × 100
                # marja = USDT_PER_TRADE (nu notional!)
                # -30% ROI = -7 USDT × 0.30 = -2.10 USDT
                max_loss_usd = -upt * getattr(config, "MAX_LOSS_PCT_EMERGENCY", 0.30)
                if unrealized < max_loss_usd:
                    roi_pct = unrealized / notional * 100
                    logger.error(
                        f"[EMERGENCY] {symbol} pierde {unrealized:.2f} USDT "
                        f"({roi_pct:.0f}% ROI) — limita: {max_loss_usd:.2f} USDT — INCHID MARKET!"
                    )
                    qty = abs(pos_amt) if pos_amt != 0 else abs(float(pos.get("qty", 0)))
                    cs  = "SELL" if pos_amt > 0 else "BUY"
                    try:
                        self.client.futures_create_order(
                            symbol=symbol, side=cs,
                            type="MARKET", quantity=qty, reduceOnly=True
                        )
                        logger.info(f"[EMERGENCY] {symbol} inchis market! PNL={unrealized:.2f}")
                        try:
                            from notifier import notify_error
                            notify_error("🚨 EMERGENCY CLOSE",
                                f"{symbol}: {unrealized:.2f} USDT ({roi_pct:.0f}% ROI)")
                        except Exception: pass
                        continue  # sarim peste SL/TP check — pozitia e inchisa
                    except Exception as ce:
                        logger.error(f"[EMERGENCY] {symbol} close error: {ce}")
