...
def _check_ema_filters(df, direction):
...
def _check_overextension(df, avg_body, direction):
    consec = 0
    for i in range(3, 3 + MAX_CONSEC_AGGR + 2):
        if i >= len(df):
            break
        candle = df.iloc[-i]
        if direction == "BULL" and candle["close"] > candle["open"] and candle["body"] >= avg_body * AGGR_FACTOR:
            consec += 1
        elif direction == "BEAR" and candle["close"] < candle["open"] and candle["body"] >= avg_body * AGGR_FACTOR:
            consec += 1
        else:
            break
