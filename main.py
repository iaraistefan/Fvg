...
class FVGBot4H:
    def __init__(self):
        ...
        logger.info(f"  Detector: GAP%≥{config.MIN_GAP_PCT*100:.2f} | AVG_BODY={config.AVG_BODY_PERIOD}")
        logger.info(f"            RSI_BULL≥{config.RSI_BULL} | RSI_BEAR≤{config.RSI_BEAR} | AGGR={config.AGGR_FACTOR}")
        logger.info(f"            Wick≤{config.MAX_WICK_RATIO} | EMA slope≥{config.EMA_MIN_SLOPE*100:.2f}%")
        logger.info(f"            MaxConsecAggr={config.MAX_CONSEC_AGGR} | EMA fast={config.EMA_FAST}, slow={config.EMA_SLOW}")
